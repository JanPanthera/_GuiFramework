...
class _ConfigHandler(StaticLoggerMixin):
    ...
    _custom_type_handlers: Dict[str, Dict[str, CustomTypeHandlerBase]] = {}
    _lock = RLock()

    StaticLoggerMixin.set_logger_details(FRAMEWORK_NAME, "_ConfigHandler")

    @classmethod
    def _add_config(cls, config_name: str, handler_config: ConfigFileHandlerConfig, custom_type_handlers: Optional[Dict[str, CustomTypeHandlerBase]] = None) -> None:
        """Add a configuration."""
        with cls._lock:
    ...
    @classmethod
    def _add_custom_type_handler(cls, config_name: str, type_: str, handler: CustomTypeHandlerBase) -> None:
        ...
        if not issubclass(handler.__class__, CustomTypeHandlerBase):
            ...
            return
        with cls._lock:
            if config_name not in cls._custom_type_handlers:
                cls._custom_type_handlers[config_name] = {}
            cls._custom_type_handlers[config_name][type_] = handler

    @classmethod
    def _add_custom_type_handlers(cls, config_name: str, handlers: Dict[str, CustomTypeHandlerBase]) -> None:
        """Add custom type handlers."""
        with cls._lock:
            for type_, handler in handlers.items():
                if issubclass(handler.__class__, CustomTypeHandlerBase):
                    if config_name not in cls._custom_type_handlers:
                        cls._custom_type_handlers[config_name] = {}
                    cls._custom_type_handlers[config_name][type_] = handler
                else:
    ...
    def _save_setting(cls, config_name: str, section: str, option: str, value: Any, auto_save: bool = True, skip_custom_type_handler: bool = False) -> None:
        ...
        with cls._lock:
            if not skip_custom_type_handler:
                value = cls._convert_value(config_name, value, "serialize")
    ...
    def _get_setting(cls, config_name: str, section: str, option: str, fallback_value: Optional[Any] = None, force_default: bool = False, skip_custom_type_handler: bool = False) -> Any:
        ...
        with cls._lock:
            ...
            return cls._convert_value(config_name, value, "deserialize") if not skip_custom_type_handler else value
        ...
        with cls._lock:
            ...
            if apply_custom_type_handlers:
                for section, options in config.items():
                    config[section] = {option: cls._convert_value(config_name, value, "deserialize") for option, value in options.items()}
            return config
    ...
    @classmethod
    def _add_variable(cls, config_name: str, variable_name: str, value: Any, section: Optional[str] = None) -> None:
        ...
        with cls._lock:
            if section:
                ...
                if value_from_file is not None:
                    value = cls._convert_value(config_name, value_from_file, "deserialize")
                else:
                    _ConfigFileHandler._save_setting(config_name, section, variable_name, cls._convert_value(config_name, value, "serialize"))
            _ConfigDynamicStore._add_variable(config_name, variable_name, value, section)

    @classmethod
    def _add_variables(cls, config_name: str, variables: Dict[str, Tuple[Any, Optional[str]]]) -> None:
    ...
    @classmethod
    def _set_variable(cls, config_name: str, variable_name: str, value: Any, save_override: bool = False) -> None:
        """Set a variable in the dynamic store."""
        if value is None:
            cls._log_warning(f"Failed to set variable \"{variable_name}\" in dynamic store: Value cannot be None.")
            return
        with cls._lock:
    ...
    @classmethod
    def _set_variables(cls, config_name: str, variables: Dict[str, Tuple[Any, bool]]) -> None:
    ...
    @classmethod
    def _unpack_value_pack(cls, value_pack: Union[Tuple[Any, Optional[str]], Any]) -> Tuple[Any, Optional[bool]]:
        """Unpack a value pack into its components."""
